...
from piphi_network_pfsense.contract import COMMANDS, ENDPOINTS, REQUIRED_ENDPOINTS
...
if behavior is None:
    ...
    if behavior.get("behaviorSchemaVersion") != "integration.behaviors.v2":
        errors.append(
            "behaviors.json must use behaviorSchemaVersion integration.behaviors.v2"
        )
    ...
    for device_index, device in enumerate(behavior.get("devices") or []):
        ...
        if device_id in device_ids:
            errors.append(
                f"behaviors.devices[{device_index}].id duplicates another device id"
            )
        if device_id:
        ...
        for action_index, action in enumerate(device.get("actions") or []):
            ...
            runtime = (
                action.get("runtime") if isinstance(action.get("runtime"), dict) else {}
            )
            safety = (
                action.get("safety") if isinstance(action.get("safety"), dict) else {}
            )
            command = str(runtime.get("command") or "").strip()
            risk_level = str(
                safety.get("riskLevel")
                or action.get("riskLevel")
                or runtime.get("riskLevel")
                or ""
            ).strip()
            if not command:
                errors.append(
    ...
    for template_index, template in enumerate(behavior.get("templates") or []):
        ...
        automation_schema_version = str(
            config.get("automation_schema_version") or ""
        ).strip()
        if (
